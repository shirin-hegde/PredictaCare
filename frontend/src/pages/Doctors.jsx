    ...
    return (
        <div>
            ...
            <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
                <button 
                    className={`py-1 px-3 border border-gray-200 rounded text-sm transition-all sm:hidden ${showFilter ? 'bg-[#5f6FFF] text-white' : ''}`} 
                ...
                <div className={`flex flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex' : 'hidden sm:flex'}`}>
                    ...
                    <p onClick={() => speciality === 'Cardiologist' ? navigate('/doctors/') : navigate('/doctors/Cardiologist')} 
                       className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Cardiologist" ? "bg-indigo-100 text-black" : ""}`}>
                        Cardiologist
                    </p>
                    <p onClick={() => speciality === 'Endocrinologist' ? navigate('/doctors/') : navigate('/doctors/Endocrinologist')} 
                       className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Endocrinologist" ? "bg-indigo-100 text-black" : ""}`}>
                        Endocrinologist
                    </p>
                </div>
